...
- [ ] **階段4（候選盤點完成，`speecht5_tailo-hokkien`已實測擱置）：TTS 模型比較** — 調研結果見 `reports/stage4_tts_candidates.md`。**BreezyVoice-Taigi 查無公開權重，無法實測**（但論文自報台語發音準確率只有59.2%，顯示台語TTS本身難度高，非資源問題）。**`speecht5_tailo-hokkien` 實測發現只吃羅馬字/英文輸入，餵漢字幾乎完全靜音**（RMS能量趨近雜訊底噪，非發音不準而是模型沒真的發聲）——因為沿用base SpeechT5的英文tokenizer沒有中文詞彙；要用就得先做「台語漢字→Tâi-lo」轉換層，會撞回辭典查表不可靠的老問題，目前先擱置不投入。剩 `MERaLiON-OmniVoice-Hokkien-TTS`（新加坡腔非台灣腔，當對照組）待測。用同一份100句測試集比較，至少兩位台語使用者評分（不能只靠自己聽，會逐漸習慣錯誤發音）
