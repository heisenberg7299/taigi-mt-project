...
- [ ] **階段4（候選盤點完成，`speecht5_tailo-hokkien`已實測擱置）：TTS 模型比較** — 調研結果見 `reports/stage4_tts_candidates.md`。**BreezyVoice-Taigi 查無公開權重，無法實測**（但論文自報台語發音準確率只有59.2%，顯示台語TTS本身難度高，非資源問題）。**`speecht5_tailo-hokkien` 實測發現只吃羅馬字/英文輸入，餵漢字幾乎完全靜音**（RMS能量趨近雜訊底噪，非發音不準而是模型沒真的發聲）——因為沿用base SpeechT5的英文tokenizer沒有中文詞彙；要用就得先做「台語漢字→Tâi-lô」轉換層(G2P)。**注意這是G2P問題，跟「華語→台語翻譯」不是同一個問題**：翻譯要處理語意/詞彙/語序，G2P輸入已經是正確台語漢字、只需決定讀音（但仍有一字多音/文白異讀/變調等難點）。目前先不投入建G2P。剩 `MERaLiON-OmniVoice-Hokkien-TTS`（新加坡腔非台灣腔，當對照組）待測，且要先確認它吃漢字還是羅馬字。已建好可重複使用的統一跑分框架 `scripts/tts_benchmark/`（12句固定測試集×5指標，新增候選只要寫一個adapter），第一次跑分：neurlang RTF~0.10、speecht5(tailo輸入)RTF~0.20-0.68明顯較慢，兩者都沒觸發靜音失敗門檻(0.75)。之後每測一個新候選都套同一組門檻，不要每次重新發明評分標準
