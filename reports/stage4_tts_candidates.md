...
### 追蹤更新：「難詞自動code-switch」的例子被撤回

開發者後來重聽，確認**原本舉的「輪椅」例子其實沒有講中文**，是聽錯了。
「MERaLiON會自動對難詞code-switch」這個發現的可信度因此下修——目前
沒有一個確認過的具體案例支持這個行為，先不當作已驗證的結論，只當作
待驗證的假說。

### 改用明確的自動判斷機制，而不是依賴MERaLiON的不確定行為

既然不能依賴「MERaLiON自己會不會切換」這種不確定的emergent行為，改成
自己寫明確的判斷邏輯：見 `scripts/mature_word_code_switch.py`（用jieba
斷詞後，逐詞判斷「這個詞pygoruut/neurlang轉不轉得出完整台語音標」，
轉不出來就切到中文語音）。

**過程中踩到一個坑**：原本判斷邏輯還加了「這個詞在不在教育部辭典裡」
當第二個訊號，實測發現「需要」「過敏」這種完全正常、pygoruut轉得出來、
neurlang講得很順的詞，也不在教育部辭典裡（辭典收錄的是特殊台語用字，
不是所有講得出來的詞的完整清單）——這個訊號拿掉了，只留pygoruut這個
有直接驗證過的訊號。

**這一版還是分兩個引擎**（台語用neurlang，中文用macOS `say`），音色不
一致的問題還在——見下一段用MERaLiON統一引擎的嘗試。

### 用MERaLiON統一處理兩種語言，同一個聲音——結果：語言差異不明顯

既然MERaLiON支援`language="nan"`跟`language="zh"`兩種參數，且voice
cloning能對兩者都套用同一個複製聲音，理論上可以用同一個模型、同一個
聲音，只切換language參數處理「盤尼西林」這種詞，不用再拼接兩個引擎。

實測：用同一組neurlang聲音複製prompt，分別以`language="zh"`跟
`language="nan"`生成同一句「你對盤尼西林敢會過敏？」。**開發者聽過
確認：聲音確實是同一個，但兩種language參數生成出來「唸起來好像也差不多」
——`zh`模式沒有明顯讓發音變成中文腔**。

**結論**：這個模型的`language="zh"`參數在voice cloning模式下，沒有
產生跟`language="nan"`有明顯可辨識差異的中文發音效果，至少在這句測試
上是這樣。這代表「用MERaLiON統一引擎+切換language參數」這條路目前
**沒有達到預期效果**，不是可靠的解法。

### 目前結論

三個方向都測過、都有明確限制：
1. **macOS say + neurlang拼接**：音色不一致（兩個不同引擎）
2. **MERaLiON統一引擎切換language參數**：音色一致，但語言差異不明顯，
   沒有真的達到「難詞用中文腔」的效果
3. **MERaLiON自動code-switch**：原本觀察到的例子已撤回，沒有可信證據

**這個方向（中台混讀TTS）目前沒有一個測出來真的可行的做法**，三條路
都卡住。在有更好的線索之前，先不繼續投入，維持現有的「pygoruut轉不出來
就是轉不出來」這個事實，不強行做語言切換的補救。
